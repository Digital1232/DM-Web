        ...
        const LEAVE_APPROVAL_CHAINS = {
            // Immanuel: Thanush â†’ Palani â†’ Nanjil (final)
            ...
            // Barath, Karthika, Thanush, Alex: Palani â†’ Nanjil (final)
                        ...
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div class="text-right">
                                    ...
                                    <p class="text-xs font-black text-amber-500">${'â˜…'.repeat(r.rating)}${'â˜†'.repeat(5-r.rating)}</p>
                                </div>
                            </div>
                        </div>
            ...
            list.innerHTML = reports.map(r => `
                <div onclick="openQcReportDetails('${r.id}')" class="p-4 hover:bg-slate-50 transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-4 cursor-pointer">
                    <div class="min-w-0">
                        ...
                        <p class="text-[10px] text-slate-400 font-bold uppercase mt-1">QC: ${escapeHtml(r.qcUser)} Â· Assignee: ${escapeHtml(r.assignee)} Â· ${new Date(r.timestamp).toLocaleDateString()}</p>
                    </div>
                    <div class="flex items-center gap-4 shrink-0">
                        <div class="text-center"><p class="text-[9px] font-bold text-slate-400 uppercase">Rating</p><p class="text-sm font-black text-amber-500">${'â˜…'.repeat(r.rating)}${'â˜†'.repeat(5-r.rating)}</p></div>
    ...
    async function openQcReportDetails(reportId) {
        ...
        if (report.mistakeItems && report.mistakeItems.length > 0) {
            ...
            mistakesHtml = `<p class="text-xs text-emerald-600 font-bold bg-emerald-50 p-3 rounded-lg border border-emerald-100 mt-4">âœ“ No issues found. Perfect score!</p>`;
        }
        ...
        content.innerHTML = `
            ...
            <div class="grid grid-cols-3 gap-3 my-4">
                ...
                <div class="bg-amber-50 p-3 rounded-xl border border-amber-100 text-center"><p class="text-[9px] font-bold text-amber-400 uppercase">Rating</p><p class="text-lg font-black text-amber-500">${'â˜…'.repeat(report.rating || 0)}</p></div>
    ...
    async function sendAutomaticAnnouncement(title, body) {
        ...
        function doBreak() {
            ...
            if (activeTaskId && !taskOnHold) {
                holdTask();
                toast('Break started â€” active task put on hold', 'info');
            } else {
        ...
        async function fetchAllJiraIssues(jql, fields = 'summary,status,priority,labels,assignee,duedate') {
            ...
            while (total === null || startAt < total) {
                ...
                console.log(`ðŸ“¡ Fetching Jira issues: startAt=${startAt}, maxResults=${maxResults}`);
        ...
        async function syncTasks(isAuto = false) {
            ...
            try {
                ...
                console.log('âœ… Token is VALID - user:', testRes.data?.emailAddress);
                ...
                console.log(`ðŸ“¡ Fetched ${issues.length} Jira issues across pages.`);
                let jiraTasks = mapJiraIssues(issues);
                const subtaskCount = jiraTasks.filter(t => t.parentId).length;
                console.log(`ðŸ“Œ Imported ${subtaskCount} Jira subtasks among ${jiraTasks.length} total tasks.`);
                console.log(`ðŸ“Š Mapped ${jiraTasks.length} tasks from Jira for ${isAuto ? 'auto-sync' : 'full-sync'}`);
                ...
                console.error('ðŸ”´ Sync exception:', e); 
                if (!isAuto) {
            ...
            if (currentTaskViewMode === 'kanban' && kanban) {
                ...
                tbody.innerHTML = filtered.map(t => {
                    ...
                    <tr class="hover:bg-slate-50 transition-colors ${activeTaskId === t.id ? 'bg-indigo-50/30' : ''} ${isOverdue ? 'bg-rose-50/30' : ''}">
                        ...
                        <td class="px-6 py-4 hidden md:table-cell text-xs text-slate-600 font-medium">${t.client || 'â€”'}</td>
        ...
        async function toggleActiveTask(id) {
            ...
            if (activeTaskId && activeTaskId !== id) {
                ...
                toast(`Task ${activeTaskId} ended â€” ${formatTime(taskSeconds)} logged`, 'info');
        ...
        function notifyIncomingMessage(msg, conv, convId) {
            ...
            if ('Notification' in window && Notification.permission === 'granted') {
                const notification = new Notification(title, { 
                    body: msg.text || (msg.attachmentName ? `ðŸ“Ž ${msg.attachmentName}` : 'New message'),
                    icon: 'img/logo.png'
                });
        ...
        function appendMessage(id, msg) {
            ...
            const edited = msg.editedAt && !msg.unsent ? ' Â· edited' : '';
            ...
            if (!msg.unsent) {
                ['ðŸ‘', 'â¤ï¸', 'ðŸ˜‚', 'ðŸŽ‰', 'ðŸ‘€'].forEach(emoji => {
        ...
        async function sendMessage() {
            ...
            try {
                ...
                const lastMsg = text || `ðŸ“Ž ${attachmentName}`;
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-start justify-between gap-3">
                                    <div>
                                        ...
                                        <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">${escapeHtml(item.authorName || 'Admin')} Â· ${date}</p>
                                    </div>
                                    ${canDelete}
                                </div>
            ...
            <div class="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-[8px] font-bold text-slate-400 uppercase mt-1">Review Time: ${formatTime(report.durationSeconds || 0)}</p>
                    </div>
                </div>
            </div>`;
        }
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-start justify-between gap-3">
                                    <div>
                                        ...
                                        <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Last Updated Â· ${date}</p>
                                    </div>
        ...
        function dprEntryRow(entry, showUser = false) {
            ...
            return `
                <div class="p-4 flex items-start justify-between gap-4 hover:bg-slate-50 transition-all">
                    <div class="min-w-0">
                        <p class="text-xs font-black text-slate-900">${showUser ? `${escapeHtml(entry.userName || 'User')} Â· ` : ''}${escapeHtml(entry.category || 'DPR')}</p>
                        <p class="text-[10px] text-slate-400 font-bold uppercase mt-1">${entry.date || ''}${entry.notes ? ` Â· ${escapeHtml(entry.notes)}` : ''}</p>
                    </div>
        ...
        function renderDetailedReport() {
            ...
            const allEvents = [
                ...periodAttendanceEvents.map(e => ({
                    ...
                    duration: e.duration ? formatTime(Math.floor(e.duration / 1000)) : 'â€”'
                })),
        ...
        function renderAnalyticsReport() {
            ...
            const peakHourLabel = peakProductivityHour ? `${peakProductivityHour[0]}:00` : 'â€”';
            ...
            try {
                ...
                sortedUsers.forEach(user => {
                    ...
                    userLogs.forEach(log => {
                        ...
                        clientGroups[client].push(`${log.taskDesc || log.taskId} - ${statusLabel}`);
                    });
                    ...
                    if (current && !processedTaskIds.has(current.taskId)) {
                        ...
                        clientGroups[client].push(`${current.taskDesc || current.taskId} - ${stateLabel}`);
                    }

                    Object.entries(clientGroups).forEach(([client, items]) => {
                        report += `${client}\n`; //
                        items.forEach((item, index) => report += `  ${index + 1}. ${item}\n`);
                        report += `\n`;
                    });
                    report += `\n`;
                });
        ...
        function setReqType(type) {
            ...
            if (type === 'leave') handleLeaveDurationChange();
                    ...
                    return `
                        <div class="flex items-center justify-between p-5 hover:bg-slate-50 transition-all border-b border-slate-50 last:border-0">
                            <div class="flex-1">
                                ...
                                ${r.approvals ? `<p class="text-[9px] text-slate-500 mt-1">${r.approvals.map((a, i) => `<span class="text-[8px] font-bold px-1 py-0.5 rounded ${a.status === 'approved' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-500'}">${a.approverName.split(' ')[0]}</span>`).join(' â†’ ')}</p>` : ''}
                            </div>
                            <div class="flex items-center gap-2">
        ...
        function loadApprovals() {
            ...
            onValue(ref(db, 'worksync/requests'), snap => {
                ...
                // Filter for requests pending approval from current user (only leave requests with approval chains)
                const pendingForMe = list.filter(([, r]) => {
                    ...
                    if (r.type === 'leave' && r.approvals && Array.isArray(r.approvals)) {
                    ...
                    // If it's a saturday or permission request (visible to all admins)
                    return r.type === 'saturday' || r.type === 'permission';
                ...
                el.innerHTML = pendingForMe.map(([id, r]) => {
                    ...
                    const approvalChainLabel = r.approvals.map((a, idx) => 
                        ...
                        <div class="flex items-center justify-between p-5 hover:bg-slate-50 transition-all border-b border-slate-50 last:border-0">
                            <div class="flex-1">
                                <div class="flex items-center gap-2 mb-2">
                                    ...
                                    <div>
                                        <p class="text-xs font-bold text-slate-900">${r.userName} â€” ${reqLabel(r)}</p>
                                ...
                                ${r.approvals.map((approval, idx) => `
                                    <div class="flex items-center gap-3">
                                        <div class="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${approval.status === 'approved' ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'}">
                                            ${approval.status === 'approved' ? 'âœ“' : approval.step}
                                        </div>
                                        <div class="flex-1">
        ...
        async function openEditLeaveModal(id) {
            try {
                ...
                document.getElementById('leave-type').value = r.leaveType || 'Casual Leave';
        ...
        async function diagnoseJira() {
            ...
            try {
                ...
                const jd = d.data || d;
                if (jd.accountId || jd.displayName) results.push('âœ“ Jira Auth: OK (' + (jd.displayName || jd.emailAddress) + ')');
                else results.push('âœ— Jira Auth: FAILED (' + (jd.errorMessages?.join('; ') || JSON.stringify(jd).slice(0, 120)) + ')');
            } catch (e) { results.push('âœ— Jira Auth: ERROR (' + e.message + ')'); }

            try {
                const d = await jiraRequest(`https://${domain}/rest/api/3/project/${projectKey}`);
                const pd = d.data || d;
                if (pd.key || pd.name) results.push('âœ“ Project "' + projectKey + '": Found (' + pd.name + ')');
                else results.push('âœ— Project "' + projectKey + '": NOT FOUND');
            } catch (e) { results.push('âœ— Project Check: ERROR (' + e.message + ')'); }
                ...
                <div class="flex items-center justify-between p-5 hover:bg-slate-50 transition-all border-b border-slate-50">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] text-slate-400 font-bold uppercase">${escapeHtml(u.role || 'User')} Â· ${escapeHtml(u.email)}</p>
                        </div>
                    </div>
    ...
    function openClientReportDetails(clientName) {
            ...
            list.innerHTML = filtered.map(t => `
                <label class="flex items-center gap-3 p-3 hover:bg-slate-100 rounded-xl cursor-pointer transition-all border border-transparent hover:border-slate-200">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <p class="text-[10px] text-slate-500 truncate">${t.status} ${t.client ? 'Â· ' + t.client : ''} ${t.duedate ? 'Â· Due: ' + new Date(t.duedate).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }) : ''} Â· Assignee: ${escapeHtml(assigneeName(t))}</p>
                    </div>
                </label>
            `).join('');
        }
        ...
        window.switchReportTab = switchReportTab; window.loadAttendanceEvents = loadAttendanceEvents; window.renderTimingReport = renderTimingReport; window.renderAnalyticsReport = renderAnalyticsReport; window.renderSummaryReport = renderSummaryReport; window.exportReportsCsv = exportReportsCsv; window.renderClientReport = renderClientReport; window.exportClientReport = exportClientReport;
