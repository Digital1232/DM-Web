...
const newFunctionsCode = `
    ...
    function openClientQcMistakeModal(taskId = '') {
        ...
        if (taskSelect) {
            const relevantTasks = (tasks || []).slice(0, 100); // Most recent tasks
            taskSelect.innerHTML = \`<option value="">Select a Task delivered or checked in QC...</option>\` +
                relevantTasks.map(t => \`<option value="\${t.id}" \${t.id === taskId ? 'selected' : ''}>[\${escapeHtml(t.id)}] \${escapeHtml(t.desc || t.title || '').substring(0, 60)} (\${t.status || 'Done'})\x3c/option>\`).join('');
        }
...
// 1. Insert new functions right after loadQcReports function
const loadQcReportsEndPos = scriptContent.indexOf('function setQcPerformanceFilter');
if (loadQcReportsEndPos !== -1) {
    scriptContent = scriptContent.substring(0, loadQcReportsEndPos) + newFunctionsCode + '\n' + scriptContent.substring(loadQcReportsEndPos);
    console.log('Inserted new Client QC Mistakes functions into script.js');
} else {
    console.error('Could not find setQcPerformanceFilter in script.js');
    process.exit(1);
}

// 2. Call loadClientQcMistakes inside loadQcReports
const loadQcReportsStartPos = scriptContent.indexOf('function loadQcReports() {');
if (loadQcReportsStartPos !== -1) {
    const nextLinePos = scriptContent.indexOf('\n', loadQcReportsStartPos) + 1;
    scriptContent = scriptContent.substring(0, nextLinePos) + '        loadClientQcMistakes();\n' + scriptContent.substring(nextLinePos);
    console.log('Added loadClientQcMistakes() trigger inside loadQcReports()');
}

// 3. Expose new functions to global window object
const windowExportsPos = scriptContent.indexOf('window.loadQcReports = loadQcReports;');
if (windowExportsPos !== -1) {
    ...
    scriptContent = scriptContent.substring(0, windowExportsPos) + exportsCode + scriptContent.substring(windowExportsPos);
    console.log('Exposed window exports in script.js');
}
