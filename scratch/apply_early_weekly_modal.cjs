...
const insertTarget = '</head>';
if (htmlContent.includes(insertTarget) && !htmlContent.includes('window.openWeeklyTaskAssigneeModal = openWeeklyTaskAssigneeModal;')) {
    htmlContent = htmlContent.replace(insertTarget, earlyScriptTag + '\n' + insertTarget);
    console.log('Inserted early script tag before </head>');
    fs.writeFileSync(htmlPath, htmlContent, 'utf8');
} else if (htmlContent.includes(insertTarget)) {
    // Check if early script is already present
    if (!htmlContent.includes('Early Weekly Task Assignment Modal Script')) {
        htmlContent = htmlContent.replace(insertTarget, earlyScriptTag + '\n' + insertTarget);
        console.log('Inserted early script tag before </head>');
        fs.writeFileSync(htmlPath, htmlContent, 'utf8');
    } else {
        console.log('Early script already present');
    }
}
